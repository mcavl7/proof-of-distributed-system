...
lista_operacoes = ["+", "-", "*", "/", "^"]
lista_topicos = ["novaOperacao", "vetorA", "operacaoA",
                 "vetorB", "operacaoB", "soma", "diferenca"]
...
class VetorException (Exception):
    ...
    def print_detail(self):
        print("Vetor recebido em " + str(self.topico) +
              " esta fora do formato aceito: " + str(self.valor[1:]))
...
class Servidor ():
    ...
    def aplicar_operacao_1_vetor(self, operacao, vetor):
        ...
        for i in range(len(vetor)):
            vetorRet.append(self.aplicar_operacao_1_num(
                vetor[i], oprRecebida, numRecebido))
        return vetorRet
    ...
    def processar_protocolo(self, cliente_mqtt, topico, mensagem):
        if (self.estadoAtual == INICIO):
            ...
            if (topico == "soma"):
                try:
                    self.vetSomaRec = self.converter_string_para_vetor(
                        topico, mensagem)
                ...
                try:
                    self.vetSubRec = self.converter_string_para_vetor(
                        topico, mensagem)
